...
def client_head_key(client, bucket, key_first_logged):
    """return the head for the key or None if not found"""
    try:
        response = client.head_object(Bucket=bucket, Key=key_first_logged)
    except ibm_botocore.exceptions.ClientError as ex:
        response_error_code = ex.response["Error"]["Code"]
        if response_error_code == '404':
            return None
        else:
            log.exception(f"Exception response for check of head_object({bucket}, {key_first_logged})")
            return None
    return response
...
def client_get_object(client, bucket, key):
    ...
    try:
      obj = client.get_object(Bucket=bucket, Key=key)
        ...
        log.error(f"client_get_object({bucket}, {key}) raised exception")
        log.exception(ex)
        return None
    return obj
...
def log_cos_object_keys(logdna_endpoint, logdna_ingestion_key, apikey, cos_crn, cos_endpoint, bucket, keys):
    ...
    for key in keys:
      obj = client_get_object(client, bucket, key)
      if obj != None:
...
class KeyFirstLogged:
    ...
    Return the KeyHeadOlder of the key that is in the object"""
    ...
    def older(self, key: str):
      """Is this key older then me, .i.e older then the first logged object"""
